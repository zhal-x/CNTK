...
void TestDictionarySerialization(size_t dictSize) 
{
    ...
    if (originalDict != deserializedDict1)
        BOOST_ERROR("original and deserialized dictionaries are not identical.");
...
void TestLargeValueSerialization(size_t numElements) 
{
    ...
    if (originalValue != deserializedValue)
        BOOST_ERROR("original and deserialized values are not identical.");
}
...
void TestLearnerSerialization(int numParameters, const DeviceDescriptor& device) 
{
    ...
    if (checkpoint1 != checkpoint2)
        BOOST_ERROR("original and restored from a checkpoint learners diverge.");
}
...
void TestFunctionSaveAndLoad(const FunctionPtr& function, const DeviceDescriptor& device)
{
    ...
    BOOST_TEST(AreEqual(function, reloadedFunction), "original and reloaded functions are not identical.");
}
...
void TestFunctionsForEquality(const DeviceDescriptor& device)
{
    ...
    BOOST_TEST(AreEqual(f1, f2), "two functions built with the same seed values are not identical.");

    auto f3 = BuildFFClassifierNet(inputVar, 3, device, /*seed*/ 2);
    auto f4 = BuildFFClassifierNet(inputVar, 3, device, /*seed*/ 3);

    BOOST_TEST(!AreEqual(f3, f4), "two functions built with different seed values are identical.");
}
...
void TestFunctionSerializationDuringTraining(const FunctionPtr& function, const Variable& labels, const MinibatchSourcePtr& minibatchSource, const DeviceDescriptor& device)
{
    ...
    BOOST_TEST(!AreEqual(classifierOutput1, classifierOutput2), "reloaded function is still identical to the original after it was trained.");
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        {
            BOOST_ERROR("original and reloaded functions are not identical.");
        }
...
void TestTrainingWithCheckpointing(const FunctionPtr& function1, const FunctionPtr& function2, const Variable& labels, const MinibatchSourcePtr& minibatchSource, const DeviceDescriptor& device)
{
    ...
    BOOST_TEST(AreEqual(function1, function2), "reloaded function is not identical to the original.");
    ...
    BOOST_TEST(!AreEqual(function1, function2), "reloaded function is still identical to the original after it was trained.");
