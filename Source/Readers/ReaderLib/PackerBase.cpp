...
{
    if (m_corpus == nullptr)
        RuntimeError("Write functionlity cannot be used with old style configurations. Please use deserializers.");


    auto& layout = minibatch.m_data.front()->m_layout;
    const auto& batch = sequences.m_data.front();

    std::vector<size_t> idToKeyMap;
    idToKeyMap.reserve(layout->GetAllSequences().size());
    ...
    for (auto& s : layout->GetAllSequences())
    {
        ...
        idToKeyMap.resize(s.seqId + 1);
        idToKeyMap[s.seqId] = batch[s.seqId]->m_key.m_sequence;
    }

    minibatch.m_idToKeyMapping = [this, idToKeyMap](const size_t i) { return m_corpus->IdToKey(idToKeyMap[i]); };
}
