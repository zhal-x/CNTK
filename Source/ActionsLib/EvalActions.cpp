...
static void DoEvalBase(const ConfigParameters& config, IDataReader& reader)
{
    ...
    size_t m_packThresholdSize = config(L"packThresholdSize", (size_t)(32 * 1024));
    ...
    SimpleEvaluator<ElemType> eval(net, MPIWrapper::GetInstance(), enableDistributedMBReading, numMBsToShowResult, 
                                   firstMBsToShowResult, traceLevel, maxSamplesInRAM, numSubminiBatches, m_packThresholdSize);
...
void DoCrossValidate(const ConfigParameters& config)
{
    ...
    size_t m_packThresholdSize = config(L"packThresholdSizeInKB", (size_t)32) * 1024;
