...
void DoCreateLabelMap(const ConfigParameters& config)
{
    ...
    for (const std::wstring& labelsName : labelNames)
    {
        ...
        dataReader.StartMinibatchLoop(minibatchSize, 0, 0, matrices.GetStreamDescriptions(), requestDataSize);
        int count = 0;
        ...
        dataReader.StartMinibatchLoop(minibatchSize, 0, 1, matrices.GetStreamDescriptions(), requestDataSize);
