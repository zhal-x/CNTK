...
public:
    ...
    vector<EpochCriterion> Evaluate(IDataReader* dataReader, const vector<wstring>& evalNodeNames, const size_t mbSize, const size_t testSize = requestDataSize)
    {
        ...
        if (useDistributedMBReading)
            dataReader->StartDistributedMinibatchLoop(mbSize, 0, 0, m_mpi->CurrentNodeRank(), m_mpi->NumNodesInUse(), inputMatrices.GetStreamDescriptions(), testSize);
        else
            dataReader->StartMinibatchLoop(mbSize, 0, 0, inputMatrices.GetStreamDescriptions(), testSize);
